    ...
    class Bullet: MovableGamePiece //constructor
    {
        ...
        public override void Move()
        {
            if (active == true) //only moves if active
            {
                ...
                if (PosValues.X < 0 || PosValues.X > GameVariables.InnerWidth - 10 || PosValues.Y < 0 || PosValues.Y > GameVariables.InnerHeight - 10) //if it would move out of frame, its reset
                {
                    active = false; //resets
                }
            }
